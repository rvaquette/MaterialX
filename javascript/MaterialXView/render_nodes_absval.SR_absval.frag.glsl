...
uniform int uv0_index;
uniform float uv_color_cool_in1;
uniform float uv_color_cool_in3;
...
void main()
{
    vec2 uv0_out = texcoord_0.xy;
    vec3 geomprop_Nworld_out = normalize(normalWorld);
    vec3 geomprop_Tworld_out = normalize(tangentWorld);
    float uv_split_outx = 0.0;
    float uv_split_outy = 0.0;
    NG_separate2_vector2(uv0_out, uv_split_outx, uv_split_outy);
    float uv_sum_out = uv_split_outx + uv_split_outy;
    vec3 uv_color_cool_out = vec3(uv_color_cool_in1,uv_split_outy,uv_color_cool_in3);
    float uv_mix_out = fract(uv_sum_out);
    vec3 uv_color_out = vec3(uv_split_outx,uv_split_outy,uv_mix_out);
    vec3 node_under_test_out = abs(uv_color_out);
    vec3 final_color_out = mix(uv_color_cool_out, node_under_test_out, uv_mix_out);
    surfaceshader SR_absval_out = surfaceshader(vec3(0.0),vec3(0.0));
    NG_standard_surface_surfaceshader_100(base, final_color_out, diffuse_roughness, metalness, specular, specular_color, specular_roughness, specular_IOR, specular_anisotropy, specular_rotation, transmission, transmission_color, transmission_depth, transmission_scatter, transmission_scatter_anisotropy, transmission_dispersion, transmission_extra_roughness, subsurface, subsurface_color, subsurface_radius, subsurface_scale, subsurface_anisotropy, sheen, sheen_color, sheen_roughness, coat, coat_color, coat_roughness, coat_anisotropy, coat_rotation, coat_IOR, geomprop_Nworld_out, coat_affect_color, coat_affect_roughness, thin_film_thickness, thin_film_IOR, emission, emission_color, opacity, thin_walled, geomprop_Nworld_out, geomprop_Tworld_out, SR_absval_out);
    out1 = vec4(mx_srgb_encode(SR_absval_out.color), 1.0);
}
